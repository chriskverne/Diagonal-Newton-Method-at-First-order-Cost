...
def train_qnn(n_layers, n_qubits, x, y, lr=0.01, num_meas_q=None):
    ...
    for time_step in range(100):
        ...
        for image, label in zip(x_t, y_t):
            ...
            curvature = np.zeros(n_layers*n_qubits*2)
            for i in range(len(params)):
                ...
                loss_plus  = cross_entropy_loss(out_plus,  label)
                ...
                loss_minus = cross_entropy_loss(out_minus, label)

                fp+=1

                curvature[i] = (loss_plus + loss_minus) - 2 * loss
                grads[i] = (loss_plus - loss_minus)/2
            
            params -= lr*grads/(curvature+1e-8) # Update params with SGD
