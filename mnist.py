...
from helper.create_qnn import create_qnn
...
def train_qnn_param_shift(x, y, n_qubits, n_layers, num_measurment_gates, num_epochs):
    ...
    #hess_fn = qml.jacobian(grad_fn)(params)
    hess_fn = qml.jacobian(lambda p, img, lab: grad_fn(p, img, lab), argnum=0)

              #qml.hessian(cost_fn, argnum=0)
    ...
    for time_step in tqdm(range(num_epochs), desc="Time step"):
        ...
        for image, label in tqdm(zip(x_t, y_t), total=len(x_t), desc=f"Epoch {time_step+1}/{num_epochs}", leave=False):
            ...
            H = hess_fn(params, image, label)
            P = params.size
            H_flat = pnp.reshape(H, (P, P))
            hdiag = pnp.reshape(pnp.diag(H_flat), params.shape)

            # increase fp by 2*n_active_params
            fp += 2*params.size 

            # Update active params only
            params -= 0.01* gradients/hdiag
